...
program
  ...
  .option('--external-log-groups <groups>', 'Comma-separated external log group names to analyze')
  .option('--include-cloudtrail', 'Include CloudTrail logs in analysis')
  .option('--discover-stack-related', 'Discover additional log groups related to the stack name')
  .option('--role-patterns <patterns>', 'Comma-separated role name patterns for external role discovery')
  ...
  .action(async (options) => {
    try {
      ...
      const config: AnalysisConfig = {
        ...
        // New external analysis options
        externalLogGroups: options.externalLogGroups ? options.externalLogGroups.split(',').map((s: string) => s.trim()) : [],
        includeCloudTrail: options.includeCloudtrail || false,
        discoverStackRelated: options.discoverStackRelated || false,
        rolePatterns: options.rolePatterns ? options.rolePatterns.split(',').map((s: string) => s.trim()) : [],
      };
      ...
      if (config.externalLogGroups && config.externalLogGroups.length > 0) {
        console.log(chalk.cyan(`External log groups: ${config.externalLogGroups.join(', ')}`));
      }

      if (config.includeCloudTrail) {
        console.log(chalk.cyan(`Including CloudTrail logs in analysis`));
      }

      if (config.discoverStackRelated) {
        console.log(chalk.cyan(`Discovering additional stack-related log groups`));
      }

      if (config.rolePatterns && config.rolePatterns.length > 0) {
        console.log(chalk.cyan(`Role patterns: ${config.rolePatterns.join(', ')}`));
      }
...
program.on('--help', () => {
  ...
  console.log();
  console.log('  # Analyze with external log groups (e.g., CloudTrail)');
  console.log('  $ iam-scoped-permissions analyze -s my-stack -r us-east-1 --external-log-groups "CloudTrail/aws-api-logs,/aws/lambda/github-actions"');
  console.log();
  console.log('  # Include CloudTrail logs and search for GitHub Actions roles');
  console.log('  $ iam-scoped-permissions analyze -s my-stack -r us-east-1 --include-cloudtrail --role-patterns "GitHubActions-*,GitHub-*"');
  console.log();
